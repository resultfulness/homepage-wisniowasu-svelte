<script lang="ts">
  ...
  afterNavigate(() => {
    ...
    year = currentPage.includes("2")
      ? currentPage.slice(currentPage.indexOf("2"))
      : "";
  });
  ...
  export let management: PersonCardData[] = [];
  export let chairpeople: PersonCardData[] = [];
</script>

<svelte:head>
  <title
    >Zespół {year == "" ? year : year.split("_").join("/")}- Wiśniowa SU |
    Samorząd Uczniowski</title
  >
</svelte:head>

<div class="title-box">
  <h1 class="title-box-text">
    ZESPÓŁ <span class="title-box-date"
      >{year == ""
        ? "2022/23"
        : year
            .split("_")
            .map((v, i) => (i === 1 ? v.slice(-2) : v))
            .join("/")}</span
    >
  </h1>
</div>
